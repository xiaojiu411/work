import {
  ...
  registerApiFun,
  newRegister,
  newValidateCodeForIndex,
  getUserByphone,
  sendSmsCodeForUser,
  findUserStatusInfo,
  newLogin,
  findCustormerStuts,
  validateUserPhoneFlag,
  alidatePhoneOrEmail,
  bindAccountByphone,
  validatePhoneSms,
  getDtJackpot,
...
// 获取登录状态之后
export const initLoginStateModule = {
  computed: {
    ...mapGetters(['initLoginState', 'isLogin'])
  },
  watch: {
    initLoginState(val) {
      if (val && !this.isLogin && window.location.href.includes('/user')) {
    ...
    playGame(apiFun, reqData, isFun) {
      ...
      apiFun(reqData).then((res) => {
        if (this.ERR_OK === res.code || this.ERR_OK_OTHER === res.code) {
          if(res.gameUrl){
            let gameUrl = res.gameUrl
            // 判断是否为游戏链接
            if (/^http/.test(gameUrl)) {
              if (isMobile) {
                window.location.href = res.gameUrl
              } else {
                tempWindow.location = res.gameUrl
              }
            } else {
              if (!isMobile) {
                tempWindow.close()
      ...
      getBankInfoList().then((res) => {
        if (this.ERR_OK_OTHER === res.code) {
          ...
          if (!this.bankList.length) {
            ...
            if(this.userInfo.phone==null && this.userInfo.email==null){
              this.$msgbox({
                title: '温馨提示',
                type: 'warning',
                message: '请先完善个人资料',
              }).then(() => {
                this.$router.push(path + '/user/account-settings')
              }).catch(e => {
                console.log(e)
              })
              return false
            ...
            this.$msgbox({
              ...
              this.$router.push(path + '/user/account-settings?query=bindBankCard')
            }).catch(e => {
              console.log(e)
            })
          }
...
export const levelImage = {
  computed: {
    levelImage() {
      return require(`@/common/image/level/${this.userInfo.level}.png`)
    },
    ...mapGetters([
      'userInfo'
    ])
...
export const newregister = {
  ...
  data() {
    return {
      phones:false,//手机登录
      uesers:false,//用户名登陆
      isPassword1:true,//短信验证码
      isPassword2:true,//密码
      isPassword3:true,//确认密码
      regValidate:{//注册60秒验证码
        codeShow: true,
        count: '',
        timer: null,
      },
      reglgimg: require('@/common/image/reg-lg.png'),//手机登录
      validateCodeForIndex: '/1.0/newValidateCodeForIndex',
      rgUserData: {//用户名注册
        registrationType:'1',
        loginname: '',
        password: '',
      ...
      rgPhoneagree: true,
      registered:false,//注册成功弹窗

      isShowbindAccountName:false,//账号
      isShowbindAccountByphone:false,//账号绑定弹框
      isShowbindAccountByphoneNumber:'',
      bindValidate:{//绑定60秒验证码
        codeShow: true,
        count: '',
        timer: null,
      },
  ...
  methods: {
    ...
    bindgetCode(phones){//账号绑定验证码
      findUserStatusInfo({phone:phones}).then((res) => {//查询用户状态
        if (this.ERR_OK === res.code) {
          sendSmsCodeForUser({phone:phones}).then((res) => {
            if (this.ERR_OK === res.code) {
              this.$message({
                type: 'warning',
                message: res.data
            ...
            if (!this.bindValidate.timer) {
              this.bindValidate.count = validtorReg.TIME_COUNT
              this.bindValidate.codeShow = false;
              this.bindValidate.timer = setInterval(() => {
                if (this.bindValidate.count > 0 && this.bindValidate.count <= validtorReg.TIME_COUNT) {
                  this.bindValidate.count--
                } else {
                  this.bindValidate.codeShow = true
                  clearInterval(this.bindValidate.timer)
                  this.bindValidate.timer = null
                }
              }, 1000)
            }
          })
    ...
    GetCode(phones){
      if (!validtorReg.phoneReg.test(phones)) {
        this.$message({
          type: 'warning',
      ...
      getUserByphone({phone:phones}).then((res) => {
//          号码已绑定账号
        if (this.ERR_OK === res.code) {
          this.$message({
            type: 'warning',
            message: '该手机号已被使用，请输入其它手机号。或者登录验证号码。'
          })
        } else {
          if(res.code == 2){//未绑定 获取验证码
            sendSmsCodeForUser({phone:phones}).then((res) => {
              if (this.ERR_OK === res.code) {
                this.$message({
                  type: 'warning',
              ...
              if (!this.regValidate.timer) {
                this.regValidate.count = validtorReg.TIME_COUNT
                this.regValidate.codeShow = false;
                this.regValidate.timer = setInterval(() => {
                  if (this.regValidate.count > 0 && this.regValidate.count <= validtorReg.TIME_COUNT) {
                    this.regValidate.count--
                  } else {
                    this.regValidate.codeShow = true
                    clearInterval(this.regValidate.timer)
                    this.regValidate.timer = null
                  }
                }, 1000)
              }
            })
    ...
    submitNewRegister() {
      if (!validtorReg.usernameReg.test(this.rgUserData.loginname)) {
        this.$message({
          type: 'warning',
          message: '登入账号只允许小写字母与数字的组合，字母开头，6-11字符之间,不能包含特殊字符'
        })
      ...
      if (!validtorReg.passwordReg.test(this.rgUserData.password)) {
        this.$message({
          type: 'warning',
          message: '密码长度为6-16个字符,以及必须含有字母和数字的组合'
        })
      ...
      if (!(this.rgUserData.rePwd === this.rgUserData.password)) {
        this.$message({
          type: 'warning',
          message: '两次密码请保持一致'
        })
      ...
      if (!validtorReg.codeReg.test(this.rgUserData.validateCode)) {
        this.$message({
          type: 'warning',
          message: '验证码有误'
        })
      ...
      newRegister(this.rgUserData).then((res) => {
        this.changeCode()
        if (this.ERR_OK === res.code) {
          this.rgUserData= {
            registrationType:'1',
    ...
    submitPhoneRegister(){
      ...
      if (!validtorReg.phoneReg.test(this.rgPhoneData.phone)) {
        this.$message({
          type: 'warning',
          message: '请输入正确格式的手机号码'
        })
      ...
      if (!validtorReg.codeReg.test(this.rgPhoneData.smsCode)) {
        this.$message({
          type: 'warning',
          message: '验证码格式有误'
        })
      ...
      getUserByphone({phone:this.rgPhoneData.phone}).then((res) => {
//          号码已绑定账号
        if (this.ERR_OK === res.code) {
          if(res.data.length==0){
            newRegister(this.rgPhoneData).then((res) => {
              if (this.ERR_OK === res.code) {
                this.rgPhoneData= {//手机注册
                  registrationType:'2',
                  phone: '',
                  smsCode: ''
                },
...
//          【账号绑定号码弹窗：输入完成账号、验证码, 验证成功之后可使用手机号登录】
            this.isShowbindAccountByphone = true
            this.isShowbindAccountByphoneNumber = this.rgPhoneData.phone
          }
        } else {
          if(res.code == 2){//未绑定--注册
            newRegister(this.rgPhoneData).then((res) => {
              if (this.ERR_OK === res.code) {
    ...
    submitbindAccountByphone(phone) {
      if (!validtorReg.oldusernameReg.test(this.bindAccData.loginname)) {
        this.$message({
          type: 'warning',
          message: '账号有误'
        })
      ...
      if (!validtorReg.codeReg.test(this.bindAccData.smsCode)) {
        this.$message({
          type: 'warning',
          message: '验证码有误'
        })
      ...
      this.bindAccData.phone = phone
      bindAccountByphone(this.bindAccData).then((res) => {
        if (this.ERR_OK === res.code) {
          this.bindAccData = {//手机绑定账号
            loginname:'',
            phone: '',
            smsCode: ''
          }
...
export const newlogin = {
  ...
  data() {
    return {
      ...
      userPwLoginData:{
        loginType:1,//账号名加密码登陆1 手机加密码登录3
        loginname:'',
        password:'',
        // validateCode:''
      },
      ...
      // checkCode: false,
      validateCodeForlg: '/1.0/newValidateCodeForIndex',

    }
  },
  computed: {
    ...mapGetters(['userInfo', 'isLogin', 'isAgent',])
  },
  methods: {
    changeCodes() {
      this.validateCodeForlg = this.validateCodeForlg + `?r=${Math.random().toFixed(5)}`
    },
    // checkStatus(){//判断密码错误次数 3
    //   findCustormerStuts({loginname:this.lgData.namePhone}).then((res)=> {
    //     if (res.code === this.ERR_OK) {
    //       if(res.data>2){
    //         this.checkCode = true
    //         this.changeCodes()
    ...
    // },
    keystatus(){//登录判断
      if(this.lgData.namePhone){
        this.uesers=true
        if (validtorReg.phoneReg.test(this.lgData.namePhone)){
          this.uesers=false
          this.phones=true
        }else {
          this.phones=false
      ...
      newLogin(datas).then((res) => {
        ...
        if (res.code === this.ERR_OK) {
          ...
          getUser().then((res) => {
            if (res.code === this.ERR_OK) {
              // 移动端
              if (isMobile && window.location.href.includes('/mobile/newlogin')) {
                this.$router.push('/mobile')
              }
              // 设置用户数据
              this.SET_USERINFO(res.data)
    ...
    submitNewLogin(){
      if(!this.lgData.namePhone.length){
        this.$message({
          type: 'warning',
          message: '请输入账户名或手机号'
        })
      ...
      if(this.uesers){//账号密码
        ...
        this.userPwLoginData.loginname = this.lgData.namePhone
        this.userPwLoginData.password = this.lgData.passWord
        // this.userPwLoginData.validateCode = this.lgData.code
        this.login(this.userPwLoginData)
      }
      if(this.phones){//验证码或密码

        if (!this.phoneLoginData.smsCode) {
          this.$message({
            type: 'warning',
            message: '请输入验证码或密码'
          })
        ...
        if(this.phoneLoginData.smsCode.length==4){//手机号+验证码
          getUserByphone({phone:this.lgData.namePhone}).then((res) => {
            if (this.ERR_OK === res.code) {//号码已绑定账号
              if(res.data.length==0){
                validateUserPhoneFlag({phone:this.lgData.namePhone}).then((res) => {
                  //校验客户手机号是否已存在并且已经通过校验（可登陆手机号） code 0 可登陆 code其他值为不可登陆
                  if (this.ERR_OK === res.code) {//可登陆手机号
                    this.phoneLoginData.phone = this.lgData.namePhone
                    this.login(this.phoneLoginData)
                  }else {//手机注册
                    newRegister({
                      registrationType:'2',
                      phone: this.lgData.namePhone,
                      smsCode: this.phoneLoginData.smsCode
                    }).then((res) => {
                      if (this.ERR_OK === res.code) {
                        this.checkLogin()
                        this.registered = true
                ...
                this.isShowbindAccountByphone = true
                this.isShowbindAccountByphoneNumber = this.lgData.namePhone
              }
            } else {
              if(res.code == 2){//未绑定
                this.$message({
                  type: 'warning',
                  message: '此号码尚未注册，请您注册后登陆'
                })
          ...
          this.PhonePwLoginData.phone = this.lgData.namePhone
          this.PhonePwLoginData.password = this.phoneLoginData.smsCode
          // this.PhonePwLoginData.validateCode = this.lgData.code
          this.login(this.PhonePwLoginData)
        }
      }
...
//          号码已绑定账号
        if (this.ERR_OK === res.code) {
          this.isShowbindAccountName = res.data
          if(res.data.length==0){
            sendSmsCodeForUser({phone:phones}).then((res) => {
              if (this.ERR_OK === res.code) {
                this.$message({
              ...
              if (!this.lgValidate.timer) {
                this.lgValidate.count = validtorReg.TIME_COUNT
                this.lgValidate.codeShow = false;
                this.lgValidate.timer = setInterval(() => {
                  if (this.lgValidate.count > 0 && this.lgValidate.count <= validtorReg.TIME_COUNT) {
                    this.lgValidate.count--
                  } else {
                    this.lgValidate.codeShow = true
                    clearInterval(this.lgValidate.timer)
                    this.lgValidate.timer = null
                  }
                }, 1000)
              }
            })
    ...
    playGameFun(item, isFun) {
      ...
      switch (item.gameCategory) {
        ...
        case 'PG':
          ...
          break
        case 'YGG':
          this.playGame(gameYGGLogin, {
            gameCode: item.gameCode,
            mode: isFun ? 'fun' : 'money',
            type: item.gameTerminal === 'PC' ? 'PC' : 'MB'
          }, isFun)
          break
      }
    },
  ...
  data() {
    return {
      ...
      allItems: [
        ...
        //   content: '存款加倍送彩金',
        //   time: '详情见专题内',
        //   img: require('@/common/image/promotion/marb-youhui.png'),
        //   link: path + "/activity/marbonus",
        //   type: []
        // },
        {
          id: 'dtv',
        ...
        },
        {
          ...
          content: '老虎机周签到送彩金',
          time: '详情见专题内',
          img: require('@/common/image/promotion/sign-youhui.png'),
          link: path + "/activity/signWeek",
          type: []
        },
        {
          id: 'dtv',
          ...
          content: '尊宝DT甜蜜合作',
          time: '详情见专题内',
          img: require('@/common/image/promotion/dtv-youhui.png'),
          link: path + "/activity/valentineDt",
          type: []
        },
        {
          id: 'vt',
          ...
          content: '每日疯狂存送',
          time: '详情见专题内',
          img: require('@/common/image/promotion/vt-youhui.png'),
          link: path + "/activity/valentine",
          type: []
        },
        {
          id: 'shoucun',
        ...
        },
        {
          ...
          time: '长期有效',
          img: require('@/common/image/promotion/upgradevip.jpg'),
          state: false,
          type: []
        },
        {
          ...
          time: config.promotion.TIME,
          img: require('@/common/image/promotion/miaocun3.jpg'),
          state: false,
          type: []
        },
        {
        ...
        },
        {
          id: 'limitTime',
          title: '尊宝限时30%-100%存送',
          content: '在特定的时间尊宝会开放30%-100%存送优惠活动，请您关注尊宝官网或者下载尊宝APP，掌握最新优惠资讯。',
          time: config.promotion.TIME,
...
export const depositModule = {
  ...
  data() {
    return {
      ...
      payWay: [],// 所有存款通道
      //所有存款类型及通道分类（注：第三方支付没有depositName属性）
      depositTypes: {
        bankCardTitle: {//银行卡title
          currentType: config.PAYWAY.yinLian,
          list: [
            {//第三方支付
              payWayType: config.PAYWAY.yinLian, //当前通道唯一标识
              type: 'ylsm',
              title: '银联扫码',//通道标题
              icon: 'saoma',//图标
              describe: '银联APP，云闪付扫码',//描述
              payWayItem: { //银联扫码通道
                payWayCurrentIdx: 0, //当前支付通道索引,默认选中第一个通道
                depositMoney: null, //存款金额
                discount: 0,//优惠为0
                accountCurrentIdx: -1,//存款金额索引，默认为空
                currentPayWay: {},//后台传过来的通道数据
                payWayType: config.PAYWAY.yinLian,//支付通道标识
                payWayList: [],//该类型支付通道集
                tooltip: {//充值说明
                  title: '银联扫码充值说明',
                  items: [
...
export const depositPageModule = {
    ...
    commonAccountList() {
      ...
      if (this.obj.payWayType === config.PAYWAY.wxSM) {
        ...
        if (payWayList.length) {
          ...
          // 通道2
          let idx2 = payWayList.findIndex((item) => {
            return item.payPlatform === 'angwx1h5'
          })
          if (idx2 === this.obj.payWayCurrentIdx) {
            res = [20, 50, 100, 200, 300, 500, null]
          }
          // 通道3
          let idx3 = payWayList.findIndex((item) => {
            return item.payPlatform === 'mfwxsm'
          })
          if (idx3 === this.obj.payWayCurrentIdx) {
            res = [50, 100, 200, 500, 1000, 2000, 3000, 5000, null]
          }
          // 通道4
          let idx4 = payWayList.findIndex((item) => {
            return item.payPlatform === 'tfwxsm'
          })
          if (idx4 === this.obj.payWayCurrentIdx) {
            res = [10, 20, 30, 50, 100, null]
          }
          // 5
          let idx5 = payWayList.findIndex((item) => {
            return item.payPlatform === 'bbcwx'
          })
          if (idx5 === this.obj.payWayCurrentIdx) {
            res = [200, 300, 500, 1000, 2000, 5000, null]
          }
          // 6
          let idx6 = payWayList.findIndex((item) => {
            return item.payCenterUrl  === '/ntt/zfb_wx'
          })
          if (idx6 === this.obj.payWayCurrentIdx) {
            this.$message({
              type: 'warning',
              message: `本支付通道只支持下列银行：中信银行、中国银行、兴业银行、上海银行、浦东发展银行、平安银行、民生银行、华夏银行、光大银行、北京银行、中国农业银行、中国建设银行`
            })
          }
          // 7
          let idx7 = payWayList.findIndex((item) => {
            return item.payPlatform === 'yzbwx'
          })
          if (idx7 === this.obj.payWayCurrentIdx) {
            res = [10, 30, 50, 100, 200, 500, null]
          }
          // 8
          let idx8 = payWayList.findIndex((item) => {
            return item.payPlatform === 'anfuwx'
          })
          if (idx8 === this.obj.payWayCurrentIdx) {
            res = [300, 500, 1000, 2000, 3000, 5000, null]
          }
          // 9
          let idx9 = payWayList.findIndex((item) => {
            return item.payPlatform === 'mfwx1sm'
          })
          if (idx9 === this.obj.payWayCurrentIdx) {
            res = [50, 100, 200, 300, 500, 1000, 2000, 3000, 5000, null]
          }
          // 10
          let idx10 = payWayList.findIndex((item) => {
            return item.payPlatform === 'syfwxh5'
          })
          if (idx10 === this.obj.payWayCurrentIdx) {
            res = [20, 30, 50, 100, null]
          }
          // 11
          let idx11 = payWayList.findIndex((item) => {
            return item.payPlatform === 'syfwxsm'
          })
          if (idx11 === this.obj.payWayCurrentIdx) {
            res = [100, 200, 500, 1000, 3000, 5000, null]
          }
          // 12
          let idx12 = payWayList.findIndex((item) => {
            return item.payPlatform === 'jbfwxh5'
          })
          if (idx12 === this.obj.payWayCurrentIdx) {
            res = [20, 30, 50, 100, 200, null]
          }
          // 13
          let idx13 = payWayList.findIndex((item) => {
            return item.payPlatform === 'ylfwx'
          })
          if (idx13 === this.obj.payWayCurrentIdx) {
            res = [100,150,200,250,300,350,400,450,500,1000,2000,3000,4000,5000 ,null]
          }

          if(this.obj.currentPayWay.payPlatform.includes('jfwx')){
            res = [500, 1000, 2000, 3000, 5000, null]
            this.jfwx = this.obj.currentPayWay.payPlatform
          }
          if(this.obj.currentPayWay.payPlatform.includes('zxfwx')){
            res = [9.95, 10, 19.9, 20, 29.85, 30, 49.75, 50, 99.5, 100, 199, 200, 298.5, 300, 497.5, 500, null]
            this.zxfwx = this.obj.currentPayWay.payPlatform
          }
        }
      }
      //微信扫码某个单一通道
      if (this.obj.payWayType === config.PAYWAY.yinLian) {
        let payWayList = this.obj.payWayList
        if (payWayList.length) {
          // 通道1
          let idx = payWayList.findIndex((item) => {
            return item.payPlatform === 'jhaiylsm'
          })
          if (idx === this.obj.payWayCurrentIdx) {
            res = [50, 60, 70, 80, 1000, 2000, 3000, 5000, 10000, null]
          }
    ...
    isShowInput() {
      ...
      switch (currentItem.payWayType) {
        ...
        case config.PAYWAY.wxSM:
          return getResult(['yfwxsm2', 'angwx1h5', 'mfwxsm', 'tfwxsm', 'bbcwx', 'anfuwx', 'yzbwx',
           'mfwx1sm', 'syfwxh5', 'syfwxsm', 'jbfwxh5' ,'bfwxsm' , 'ylfwx' ,this.jfwx ,this.zxfwx])
        // 支付宝扫码
        case config.PAYWAY.zfbSM:
          return getResult(['yfzfbsm','bwzfb2h5','bbczfb','zlzfbsm','zlzfbh5'])
        // 支付宝扫码
        case config.PAYWAY.yinLian:
          return getResult(['jhaiylsm'])
        // qq扫码
        case config.PAYWAY.qq:
          return getResult(['anfuqq','jfqq'])
      }
  ...
  methods: {
    ...
    commonDeposit(currentPayWay) {
      ...
      //限制通道
      if(this.obj.currentPayWay.payCenterUrl=='/kht/zfb_wx' || this.obj.currentPayWay.payCenterUrl=='/qsx/zfb_wx'){
        reqData.bankCode = 'PCBC'
      }
      ...
      commonDeposit(reqData).then((res) => {
        if (this.ERR_OK === res.code) {
          ...
          if (type === '11') {// 支付宝pc
            tempWindow.location = `${this.baseUrl}html/pay-zfb-code-pc.html?query=${url}&price=${data}`
            return
          }

          if (type === '10') {// 支付宝wap
            window.location.href = `${this.baseUrl}html/pay-zfb-code.html?query=${url}&price=${data}`
            return
          }
          ...
          if (type === '8') {// ajax请求页面链接
            if (isMobile) {
              window.location.href = `${this.baseUrl}html/postPay.html?query=${JSON.stringify(res.data)}`
            } else {
              tempWindow.location = `${this.baseUrl}html/postPay.html?query=${JSON.stringify(res.data)}`
            }
            return
          }
          if (type === '4') {// 截取字符串script中的支付链接
            console.log(data)
            if (isMobile) {
              window.location.href = data.match(/.+window.location.href='(.+)'.?/)[1]
            } else {
              tempWindow.location = data.match(/.+window.location.href='(.+)'.?/)[1]
            }
            return
          }
    ...
    secondsDeposit(currentPayWay, {repeat, deleteLastOneOrder}) {//秒存
      ...
      // 是否需要获取小数点
      if (banktype === 4 || banktype === 6 || banktype === 7 || banktype === 22 || banktype === 66) {
      ...
      getNewdeposit(reqData).then((res) => {
        if (this.ERR_OK === res.code) {
          ...
          if (res.data.massage == '使用秒存请先绑定真实姓名！') {//秒存限制
              this.$msgbox({
                title: '温馨提示',
                type: 'warning',
                message: '请先完善个人资料',
...
export const accountSettingsModule = {
  ...
  created() {
    ...
    this.baseInfo.email = this.userInfo.email
  ...
  methods: {
    ...
    //修改信息
    changeInfo() {
      let {qq, microchannel, birthday, aliasName, email} = this.baseInfo
      if (!birthday) {
        this.$message({
          type: 'warning',
          message: '请输入生日信息'
      ...
      if ((baseInfoOld.qq === qq) && (baseInfoOld.microchannel === microchannel) && (baseInfoOld.birthday === birthday) && (baseInfoOld.aliasName === aliasName) && (baseInfoOld.email === email)) {
        this.$message({
          type: 'warning',
          message: '资料没有异动'
        })
...
// 账户设置 基本资料&修改密码
export const newaccountSettingsModule = {
  mixins: [logOut, checkLogin],
  data() {
    return {
      tabTitleList: [
      ...
      setPhoneValidate:{//基本资料短信60秒验证码
        codeShow: true,
        count: '',
        timer: null,
      },
      bindPhoneValidate:{//绑定60秒验证码
        codeShow: true,
        count: '',
        timer: null,
      },
      bindPhoneAccData: {//手机绑定账号
        loginname:'',
        phone: '',
        smsCode: ''
      },
      isShowPhoneOrEmail:false,
      isPhoneBindCheck:false
    }
  },
  computed: {
    ...mapGetters(['userInfo'])
  ...
  mounted() {
    //获取用户信息接口执行后
    this.baseInfo.phone = this.userInfo.phone
    this.baseInfo.email = this.userInfo.email
    this.baseInfo.user_remark = this.userInfo.user_remark
    this.baseInfo.qq = this.userInfo.qq
    this.baseInfo.microchannel = this.userInfo.microchannel
    this.baseInfo.aliasName = this.userInfo.aliasName
    this.baseInfo.birthday = this.userInfo.birthdayStr
    this.baseInfoOld = Object.assign({}, this.baseInfo)
    this._isPhoneBindCheck()
  },
  // 每次激活重新调用
  activated() {
    if (this.$route.query.query === 'bindBankCard') {
  ...
  methods: {
    _isPhoneBindCheck(){
      validateUserPhoneFlag({phone:this.userInfo.phone}).then((res) => {
        //校验客户手机号是否已存在并且已经通过校验（可登陆手机号） code 0 可登陆 code其他值为不可登陆
        if (this.ERR_OK === res.code) {//可登陆手机号
         this.isPhoneBindCheck = true
        }
      })
    },
    //验证以手机登录
    _isShowPhoneOrEmail(){
      this.isShowPhoneOrEmail = true
    },
    //验证以手机登录的验证码
    bindGetCode(){
      ...
      findUserStatusInfo({phone:this.userInfo.phone}).then((res) => {//查询用户状态
        if (this.ERR_OK === res.code) {
          sendSmsCodeForUser({phone:this.userInfo.phone}).then((res) => {
            if (this.ERR_OK === res.code) {
              this.$message({
                type: 'warning',
                message: res.data
            ...
            if (!this.bindPhoneValidate.timer) {
              this.bindPhoneValidate.count = validtorReg.TIME_COUNT
              this.bindPhoneValidate.codeShow = false;
              this.bindPhoneValidate.timer = setInterval(() => {
                if (this.bindPhoneValidate.count > 0 && this.bindPhoneValidate.count <= validtorReg.TIME_COUNT) {
                  this.bindPhoneValidate.count--
                } else {
                  this.bindPhoneValidate.codeShow = true
                  clearInterval(this.bindPhoneValidate.timer)
                  this.bindPhoneValidate.timer = null
                }
              }, 1000)
            }
          })
    ...
    bindGetCodes(){
      ...
      getUserByphone({phone:this.baseInfo.phone}).then((res) => {
        if (this.ERR_OK === res.code) {//号码已绑定账号
          this.$message({
            type: 'warning',
            message: '此号码已经被使用，请输入其他号码'
          })
        } else {
          if(res.code == 2) {//未绑定
            sendSmsCodeForUser({phone: this.baseInfo.phone}).then((res) => {
              if (this.ERR_OK === res.code) {
                this.$message({
                  type: 'warning',
                  message: res.data
              ...
              if (!this.setPhoneValidate.timer) {
                this.setPhoneValidate.count = validtorReg.TIME_COUNT
                this.setPhoneValidate.codeShow = false;
                this.setPhoneValidate.timer = setInterval(() => {
                  if (this.setPhoneValidate.count > 0 && this.setPhoneValidate.count <= validtorReg.TIME_COUNT) {
                    this.setPhoneValidate.count--
                  } else {
                    this.setPhoneValidate.codeShow = true
                    clearInterval(this.setPhoneValidate.timer)
                    this.setPhoneValidate.timer = null
                  }
                }, 1000)
              }
            })
    ...
    //验证以手机登录提交
    submitbindAccByphone(){
      if (!this.bindPhoneAccData.smsCode) {
        this.$message({
          type: 'warning',
          message: '请输入验证码'
        })
      ...
      this.bindPhoneAccData.loginname =  this.userInfo.loginname
      this.bindPhoneAccData.phone =  this.userInfo.phone
      alidatePhoneOrEmail(this.bindPhoneAccData).then((res) => {
        if (this.ERR_OK === res.code) {
          this.$message({
            type: 'warning',
            message: res.data
    ...
    setPwd() {
      ...
      setLoginPassword(this.setpwd).then((res) => {
        if (this.ERR_OK === res.code) {
          this.$message({
            type: 'info',
            message: '设置成功',
    ...
    //修改信息
    changeInfo() {
      this.baseInfo.accountName = this.userInfo.accountName
      if(this.userInfo.phone){
        this.baseInfo.phone = this.userInfo.phone
      }
      let {accountName ,email ,phone ,smsCode ,qq, microchannel, birthday, aliasName} = this.baseInfo
      if (!accountName) {
        this.$message({
          type: 'warning',
          message: '请绑定真实姓名'
      ...
      let baseInfoOld = this.baseInfoOld
      if ((baseInfoOld.accountName === accountName) && (baseInfoOld.phone === phone) && (baseInfoOld.smsCode === smsCode) && (baseInfoOld.qq === qq) && (baseInfoOld.microchannel === microchannel) && (baseInfoOld.birthday === birthday) && (baseInfoOld.aliasName === aliasName)) {
        this.$message({
          type: 'warning',
          message: '资料没有异动'
        })
      ...
      newEditUserInfo(this.baseInfo).then((res) => {
        if (this.ERR_OK === res.code) {
          this.checkLogin(this.userInfo.loginname)
          if (res.data === 'success') {
            res.data = '资料修改成功'
          ...
          validateUserPhoneFlag({phone:this.baseInfo.phone}).then((res) => {
            //校验客户手机号是否已存在并且已经通过校验（可登陆手机号） code 0 可登陆 code其他值为不可登陆
            if (this.ERR_OK === res.code) {//可登陆手机号
              this.isPhoneBindCheck = true
            }
...
// app下载code
export const appDownLoadCode = {
  data() {
    return {
      ewma: require('@/common/image/zb-app.png'),
      ewmi: require('@/common/image/zb-app.png')
    }
  },
  computed: {
    ...mapGetters(['isLogin'])
  ...
  methods: {
    appDLcode() {
      ...
      let isEmpty = 0
      let appDownloadUrl = 'https://www.188zun.com'
      //step 2 判断代理域名
              ...
              if (this.ERR_OK === res.code) {
                if(res.data.length){
                  res.data.forEach((obj) => {
                    if (obj.plat=='iOS'){
                      this.ewmi = obj.qrCodeUrl
                    }
                    if (obj.plat=='android'){
                      this.ewma = obj.qrCodeUrl
                    }
                  })
                }
              }
